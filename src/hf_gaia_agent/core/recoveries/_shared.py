...
def invoke_recovery_tool(
    *,
    context: RecoveryExecutionContext,
    tool_name: str,
    tool_args: dict[str, Any],
    trace_label: str = "recovery",
) -> str:
...
def try_search_recovery(
    *,
    context: RecoveryExecutionContext,
    query: str,
    max_results: int = 5,
    budget: RecoveryAttemptBudget | None = None,
...
def recovery_result_from_records(
    question: str,
    ...
    for candidate_url in ordered_candidate_urls:
        for query in filtered_queries:
            if budget is not None and not budget.consume_fetch():
                return None
            found_text = invoke_recovery_tool(
                context=context,
            ...
            result = recovery_result_from_records(
                context.state["question"],
                [record],
                expected_reducer=expected_reducer,
            )
            if result:
                return with_recovery_traces(result, context=context)
    return None


def try_fetch_recovery(
    *,
    context: RecoveryExecutionContext,
    candidate_urls: list[str],
    expected_reducer: str,
