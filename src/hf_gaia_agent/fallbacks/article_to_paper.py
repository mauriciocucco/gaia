"""Article-to-paper award identifier fallback resolver."""
...
class ArticleToPaperFallback:
    ...
    def run(self, state: AgentState) -> dict[str, Any] | None:
        ...
        # Expand search with more queries
    ...
    def _identifier_candidate_urls(
        ...
        for url in candidates:
            ...
            if not hostname:
                continue
            if publisher_domains and any(
                hostname.endswith(domain) for domain in publisher_domains
            ):
                continue
            urls.append(url)
